...
function startGame() {
    ...
    wall_timer = setInterval(createWall, 5000);
}
...
function haveWall(unit) {
    var check = true;
    var wall_classes = unit.getAttribute('class').split(' ');
    var wallUnit = document.querySelector('wall_unit');
    if (wall_classes == wallUnit) {
        check = false;
    }
    return check;
}
